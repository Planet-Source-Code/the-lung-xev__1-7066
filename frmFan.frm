...
Private Sub cmdGetList_Click()
    ...
    Dim FanSet As SWbemObjectSet
    Set FanSet = Namespace.InstancesOf("Win32_Fan")
    
    For Each Fan In FanSet
        ' Use the RelPath property of the instance path to display the disk
        lstFan.AddItem Fan.Path_.RelPath
    Next
...
Private Sub lstFan_Click()
    ...
    SelectedItem = lstFan.List(lstFan.ListIndex)
    Set Fan = Namespace.Get(SelectedItem)

    Value = Fan.ActiveCooling
    lstData.AddItem Left("Build Number" & Space(35), 35)
    lstData.List(0) = lstData.List(0) & CStr(Value)
    
    Value = Fan.Availability
    ...
    Value = Fan.ConfigManagerErrorCode
    ...
    Value = Fan.PowerManagementSupported
