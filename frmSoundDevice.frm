...
Begin VB.Form frmSoundDevice 
   ...
   Begin VB.CommandButton cmdGetList 
      ...
      Width           =   975
   End
   Begin VB.ListBox lstSoundDevice 
      BeginProperty Font 
         Name            =   "Courier"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmSoundDevice"
...
Private Sub cmdGetList_Click()
    ...
    Dim SoundDevice As SWbemObject
   
    'Clear current
    lstSoundDevice.Clear
    
    Me.MousePointer = vbHourglass
      
    'Enumerate the instances
    Dim SoundDeviceSet As SWbemObjectSet
    Set SoundDeviceSet = Namespace.InstancesOf("Win32_SoundDevice")
    
    For Each SoundDevice In SoundDeviceSet
        ' Use the RelPath property of the instance path to display the disk
        lstSoundDevice.AddItem SoundDevice.Path_.RelPath
    Next
...
Private Sub lstSoundDevice_Click()
    On Error Resume Next
    ...
    SelectedItem = lstSoundDevice.List(lstSoundDevice.ListIndex)
    Set SoundDevice = Namespace.Get(SelectedItem)
    
    Value = SoundDevice.Availability
