VERSION 5.00
Begin VB.Form frmDesktopMonitor 
   ...
   ClientTop       =   330
   ClientWidth     =   8910
   Icon            =   "frmDesktopMonitor.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   4665
   ...
   Begin VB.ListBox lstData 
      ...
      Width           =   8655
   End
   Begin VB.ListBox lstDesktopMonitor 
      BeginProperty Font 
         Name            =   "Courier"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmDesktopMonitor"
...
Private Sub cmdGetList_Click()
    ...
    Dim DesktopMonitor As SWbemObject
   
    'Clear current
    lstDesktopMonitor.Clear
    
    Me.MousePointer = vbHourglass
      
    'Enumerate the instances
    Dim DesktopMonitorSet As SWbemObjectSet
    Set DesktopMonitorSet = Namespace.InstancesOf("Win32_DesktopMonitor")
    
    For Each DesktopMonitor In DesktopMonitorSet
        ' Use the RelPath property of the instance path to display the disk
        lstDesktopMonitor.AddItem DesktopMonitor.Path_.RelPath
    Next
...
Private Sub lstDesktopMonitor_Click()
    On Error Resume Next
    ...
    SelectedItem = lstDesktopMonitor.List(lstDesktopMonitor.ListIndex)
    Set DesktopMonitor = Namespace.Get(SelectedItem)
    
    Value = DesktopMonitor.Availability
    ...
    Value = DesktopMonitor.Bandwidth
    lstData.AddItem Left("Build Number" & Space(35), 35)
    lstData.List(1) = lstData.List(1) & CStr(Value) & "megahertz"

    Value = DesktopMonitor.Caption
    lstData.AddItem Left("Caption" & Space(35), 35)
    lstData.List(2) = lstData.List(2) & CStr(Value)

    Value = DesktopMonitor.ConfigManagerErrorCode
    ...
    Value = DesktopMonitor.PowerManagementSupported
    lstData.AddItem Left("PNP Device ID" & Space(35), 35)
    lstData.List(20) = lstData.List(20) & CStr(Value)

    Value = DesktopMonitor.ScreenHeight
    lstData.AddItem Left("Screen Height" & Space(35), 35)
    lstData.List(21) = lstData.List(21) & CStr(Value)

    Value = DesktopMonitor.ScreenWidth
    lstData.AddItem Left("Screen Width" & Space(35), 35)
    lstData.List(22) = lstData.List(22) & CStr(Value)

    Value = DesktopMonitor.Status
    lstData.AddItem Left("Status" & Space(35), 35)
    lstData.List(23) = lstData.List(23) & CStr(Value)

    Value = DesktopMonitor.StatusInfo
    lstData.AddItem Left("Status Info" & Space(35), 35)
    Select Case Value
        Case 1: lstData.List(24) = lstData.List(24) & "Other"
        Case 2: lstData.List(24) = lstData.List(24) & "Unknown"
        Case 3: lstData.List(24) = lstData.List(24) & "Enabled"
        Case 4: lstData.List(24) = lstData.List(24) & "Disabled"
        Case 5: lstData.List(24) = lstData.List(24) & "Not Applicable"
    End Select

    Value = DesktopMonitor.SystemCreationClassName
    lstData.AddItem Left("System Creation Class Name" & Space(35), 35)
    lstData.List(25) = lstData.List(25) & CStr(Value)

    Value = DesktopMonitor.SystemName
